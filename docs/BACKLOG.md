...
**Current state:** 1.16.0 · 15 skills · 121 tests green · one CNL-P format with no profile
...
the standard and the profiles through `ai-factory adr format`, never by path. `cnlp-kit/` takes
the format to other skill repositories; it is authoring material and never ships in the package.
