...
- author `profiles/skill.md` from this repository's own vocabulary before any body is touched
...
preconditions:
- `docs/cnlp-format.md` and `profiles/skill.md` exist in this repository
- the working tree is clean, so the rewrite is reviewable as its own diff

scope:
- act on skill bodies, on `profiles/skill.md`, and on the shared rubric document
...
- do not add a block that `profiles/skill.md` does not declare: the profile is edited first, or the content goes in a declared block
...
outputs:
- an authored `profiles/skill.md`, its `custom_sections` filled from the inventory
...
workflow:
1. read `docs/cnlp-format.md` in full, then `profiles/skill.md`: the rules and the values are read together, neither is complete alone
...
5. fill `custom_sections:` in `profiles/skill.md` with the names that survive, and delete its `- none` line
...
invocation:
- Claude Code: `/cnlp-migrate <skills-directory>`
- Codex: `$cnlp-migrate <skills-directory>`
