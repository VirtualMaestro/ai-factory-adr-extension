...
- be the by-ADR-name shortcut for the manual "resolve the plan, then run `aif-improve`" flow documented in `adr-plan`
...
- do not advance the ADR: that is `adr-implement` and `adr-finalize`
...
2. stop and recommend `adr-plan` first when there is no active plan: there is nothing to improve yet
...
status_footer:
  format: "✔ adr-plan-improve · ADR: <adr-id> [<status>] · Plan: <plan-id> (<plan-status>)"
  source: `ai-factory adr status <adr-file>`

invocation:
- Claude Code: `/adr-plan-improve @adr-file`
- Codex: `$adr-plan-improve @adr-file`
