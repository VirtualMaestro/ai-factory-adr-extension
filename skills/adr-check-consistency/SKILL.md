...
- be the decision-against-decision counterpart of `adr-verify-all`, which checks decisions against code
...
- a `proposed` or `draft` ADR is not a rule yet: 2 drafts in flight stop conflicting at `adr-accept`, which reads the same digest before the transition
...
follow_up:
- contradiction where 1 ADR is `accepted` and the other `active`: recommend `adr-improve` on the `accepted` one, which no code implements yet, while PRD §18.3 protects the `active` one
...
- redundant: name the overlap and stop; `adr-supersede` takes an explicit old and new ADR, which coverage alone does not supply
- shared-area: no action; the row exists so the author of each ADR knows about the neighbour
- `adr-supersede` runs after the operator names the pair, never before

status_footer:
  format: "✔ adr-check-consistency · pairs: <n> · contradiction <a> · redundant <b> · shared-area <c>"
  source: the verdicts you assigned

invocation:
- Claude Code: `/adr-check-consistency`
- Codex: `$adr-check-consistency`
