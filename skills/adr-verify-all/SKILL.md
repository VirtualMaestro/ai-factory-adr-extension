---
name: adr-verify-all
description: Verify every active ADR against the implemented code in one sweep — run adr-verify's checks per ADR and report a conformance table. Read-only; reports verdicts, never rewrites.
---
...
- run the `adr-verify` conformance check over every active ADR in one sweep
- be the batch companion to `adr-verify`: same per-ADR judgment, one aggregated report
...
targeting_rationale:
- an `accepted` ADR records a decision that no code implements yet, and a half-implemented one stays `accepted` until `adr-finalize` activates it, so there is no finished code to judge
- those belong to `adr-next` and `adr-implement`, not to a conformance sweep
- to check a single `accepted` or `active` ADR, use `adr-verify`
...
follow_up:
- drift or contradiction: recommend `adr-improve` for that ADR, to return it to draft and re-decide
- missing anchors on an active ADR: recommend refreshing its `code:` array by re-running `adr-finalize` or editing the anchors via refine
- partial: point back to `adr-implement` for that ADR

status_footer:
  format: "✔ adr-verify-all · active: <n> · implemented <a> · partial <b> · drift <c> · not-implemented <d> · doc-only <e>"
  source: the verdicts you assigned

invocation:
- Claude Code: `/adr-verify-all`
- Codex: `$adr-verify-all`
