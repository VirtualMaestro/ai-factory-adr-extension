...
- re-runnable, unlike `adr-finalize`, which verifies once at activation against the linked plan
...
follow_up:
- drift or contradiction: recommend `adr-improve` to return the ADR to draft and re-decide, mirroring `adr-finalize`
- missing anchors on an active ADR: recommend refreshing the `code:` array by re-running `adr-finalize` or editing the anchors via refine
- partial or not-implemented on an accepted ADR: the implementation work is unfinished, point back to `adr-implement`

status_footer:
  format: "✔ adr-verify · ADR: <adr-id> [<status>] · Plan: <plan-id or none> · verdict: <verdict>"
  source: `ai-factory adr status <adr-file>` plus the verdict you assigned

invocation:
- Claude Code: `/adr-verify @adr-file`
- Codex: `$adr-verify @adr-file`
