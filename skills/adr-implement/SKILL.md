...
- leave finalization to `adr-finalize`: completing implementation is not finalizing the decision
...
status_footer:
  format: "✔ adr-implement · ADR: <adr-id> [<status>] · Plan: <plan-id> (<plan-status>)"
  source: `ai-factory adr status <adr-file>`

invocation:
- Claude Code: `/adr-implement @adr-file`
- Codex: `$adr-implement @adr-file`
