...
- do not act on implementation intent: the operator invokes `adr-next` or the plan skills
...
15. update the ADR
16. when this pass changed no `decision:`, `scope:`, `constraints:` or `rules:` line, refinement is done: recommend `adr-accept` and run no further pass
...
status_footer:
  format: "✔ adr-improve · ADR: <adr-id> [<status>] · Plan: <plan-id or none>"
  source: `ai-factory adr status <adr-file>`

invocation:
- Claude Code: `/adr-improve @adr-file`
- Codex: `$adr-improve @adr-file`
