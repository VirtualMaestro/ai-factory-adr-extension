...
- the ADR actually requires implementation; documentation-only decisions skip straight to `adr-finalize`
...
- do not fill `evidence:`; `plan:` filled with `evidence:` empty is the pending state, and `adr-finalize` sets `evidence: implemented` later
...
3. stop when that command reports an `issues:` entry for this ADR, and recommend `adr-improve`: a plan built on a decision that failed to parse rests on nothing
...
- do not reimplement improve logic for ADRs; `adr-improve` avoids `aif-improve` only because it works on the decision, a different task
- the easy path is `adr-plan-improve @adr-file`, which applies `aif-improve` to this plan while you name the ADR, so you never track the plan's path
...
status_footer:
  format: "✔ adr-plan · ADR: <adr-id> [<status>] · Plan: <plan-id> (<plan-status>)"
  source: `ai-factory adr status <adr-file>`

invocation:
- Claude Code: `/adr-plan @adr-file`
- Codex: `$adr-plan @adr-file`
