...
6. stop and recommend `adr-improve` or `adr-supersede` on the existing ADR when one already covers this decision
...
13. leave `status: proposed`; acceptance happens later via `adr-improve` then `adr-accept`
14. report the status footer

status_footer:
  format: "✔ adr-propose · ADR: <adr-id> [proposed] · Plan: none"
  source: `ai-factory adr status <adr-file>`, where a fresh proposal has no plan

invocation:
- Claude Code: `/adr-propose <topic>`
- Codex: `$adr-propose <topic>`
